...
namespace ClickSync
{
    class DBHelper{
        ...
        public async Task<int> GetNumberOfChangesFromDB(){
            try
            {
                var sqlCommand = "SELECT COUNT(*) FROM [dbo].[Pratim_pp] WHERE ClickObjectID IS NOT NULL AND ClickSynced=0 AND (NOT RetirementDate <= GETDATE() OR RetirementDate IS NULL)";
                SqlCommand cmd = new SqlCommand(sqlCommand, _conn);
                return (int)cmd.ExecuteScalar();
             }
            catch (System.Exception ex)
        ...
        public async Task<List<ClickUser>> GetRetirementsFromDB(int numberOfUsersToGet){
            try
            {
                var sqlCommand = "SELECT TOP (@rows) * FROM [dbo].[Pratim_pp] WHERE ClickObjectID IS NOT NULL AND ClickSynced=0 AND RetirementDate <= GETDATE()";
        ...
        public async Task<List<ClickUser>> GetUsersFromDB(int numberOfUsersToGet)
        {
            ...
            {
                List<ClickUser> clickUsers = new List<ClickUser>();
                var sqlCommand = "SELECT TOP (@rows) * FROM [dbo].[Pratim_pp] WHERE ClickObjectID IS NOT NULL AND ClickSynced=0 AND (NOT RetirementDate <= GETDATE() OR RetirementDate IS NULL)";
                SqlCommand cmd = new SqlCommand(sqlCommand, _conn);
                cmd.Parameters.AddWithValue("@rows", numberOfUsersToGet);
                SqlDataReader reader = cmd.ExecuteReader();
                while (reader.Read())
        ...
        {
            var sqlCommand = "UPDATE [dbo].[Pratim_pp] SET ClickSynced=1 WHERE TZ=@tz";
